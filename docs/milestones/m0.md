...
| [C4b #24](https://github.com/GhostFlying/cove/issues/24) | Mixed-version protocol acceptance                   | F1          | `tests/compatibility/m0/**`                                             |
...
| C3 / C4a / C4b / C5                        | C3 / F1 / C4b / C5；F1 在 C3 后冻结完整 A7 journey，防止后续旧端无能力而无法完成兼容验收                   |
...
F1 冻结真实 source SHA、lock/profile、摘要及可重建产物。C4b 候选必须是后续不同 SHA，独立构建/加载新旧代码；不能用当前 schema 的两个 alias、缺能力的旧 stub 或人为更改版本标签冒充兼容验证。M0 内参考版本不称作已发布产品。
