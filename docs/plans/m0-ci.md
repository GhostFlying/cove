...
| C3 / terminal test owner               | C2；P 经 T1 反馈冻结的 profile/顺序/恢复契约；T 权威模型与客户端 adapter；已完成 P4 两 view harness 及 T4 恢复路径 | 两个真实浏览器 harness 实例共享生产 adapter，运行 A1-A6；反例注入证明断言会失败。功能与测试同步接入必需 check。                                                                                                                                                                                      |
