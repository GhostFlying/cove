...
| 实现           | TraeX GPT-5.6 Sol high / GPT-6 Sol high / GPT-6 Luna max | 实现任务及必要的本地测试；适合的非规划任务优先分给 TraeX         |
| 独立测试       | 独立的同类非规划执行者                                   | 从验收条件验证边界、失败路径和集成行为                           |
| 独立 review    | 独立的同类非规划执行者                                   | 审查最终变更与证据，不由该变更的实现者或测试作者自审             |
| 集成与运行事务 | 明确分配的同类非规划执行者                               | 维护文档、处理冲突、执行 GitHub/CI 操作；实质改动重新验证/review |
