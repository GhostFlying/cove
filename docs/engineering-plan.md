...
协调者持续维护进度、分派、集成和验收，不必亲自解决所有依赖或冲突。

| 角色                | 模型                                       | 责任                                                      |
| ------------------- | ------------------------------------------ | --------------------------------------------------------- |
| 模块规划            | GPT-6 Astra high                           | 写模块 plan、拆任务依赖图、定义接口和验收条件             |
| 实现                | GPT-6 Sol high / GPT-6 Luna max            | 实现任务及必要的本地测试                                  |
| 独立测试            | 新的 GPT-6 Sol high / GPT-6 Luna max agent | 从验收条件验证边界、失败路径和集成行为                    |
| 可选实现/测试执行器 | 本地 TraeX GPT-5.6 Sol high，经 warmpool   | 使用明确的模型和 high 参数，不代替规定的 planner/reviewer |
| 独立 review         | 新的 GPT-6 Sol high agent                  | 审查最终变更与证据，不由该变更的实现者或测试作者自审      |
| 依赖与冲突协调      | 协调者或 GPT-6 Sol high                    | 维护契约一致性、处理集成冲突；实质改动重新验证和 review   |
...
产品取舍、架构调整、范围变化或验收标准改变，由协调者整理选项、建议、影响和阻塞任务后交用户评估决定。
普通实现细节和已批准契约内的冲突由执行者处理。等待决定时仅阻塞相关依赖，其他 ready 任务继续。
协调者已获授权：满足独立测试、独立 review 和必需 CI 后自主合并；该授权不包含跳过卡口、发布部署或进入下一 milestone。
...
每个 milestone 先由各模块 planner 拆分任务，再由协调者合并为同一张无环依赖图。
...
协调者在主 checkout 维护调度；实际调度同时服从工具槽位限制。不得让外部 CLI 或递归派发绕过总预算。
...
冲突可由 GPT-6 Sol high 解决，解决后的代码属于新变更，必须重新验证并由独立 reviewer 审阅。
...
review 结论及对应 SHA 记录在 PR，由协调者检查；CI 是机器卡口，agent 身份独立性由调度记录证明。
...
长期工作不依赖完整聊天记录。协调者维护 [当前 handoff](handoff.md)，各任务维护自己的 plan 与交接记录。
